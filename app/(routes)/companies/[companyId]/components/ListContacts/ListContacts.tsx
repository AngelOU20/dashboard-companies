import React from "react";
import { ListContactsProps } from "./ListContacts.type";
import { auth } from "@clerk/nextjs";
import { Separator } from "@/components/ui/separator";
import { Mail, Phone } from "lucide-react";
import { redirect } from "next/navigation";
import { db } from "@/lib/db";

export const ListContacts: React.FC<ListContactsProps> = async ({
  company,
}) => {
  const { userId } = auth();

  if (!userId) {
    return redirect("/");
  }

  const contacts = await db.contact.findMany({
    where: {
      company: {
        id: company.id,
      },
    },
  });

  if (contacts.length === 0) {
    return <p>Actualmente no dispones de ningún contacto</p>;
  }

  return (
    <div>
      <div className="mb-2 grid grid-cols-3 p-2 gap-x-3 items-center justify-between px-4 bg-slate-400/20 rounded-lg">
        <p>Name</p>
        <p>Role</p>
        <p className="text-right">Contact</p>
      </div>

      {contacts.map((contact) => (
        <React.Fragment key={contact.id}>
          <div className="grid grid-cols-3 items-center justify-between px-4">
            <p>{contact.name}</p>
            <p>{contact.role}</p>
            <div className="flex items-center gap-x-6 justify-end">
              {contact.phone ? (
                <a href={`telto: ${contact.phone}`} target="_blank">
                  <Phone className="w-4 h-4" />
                </a>
              ) : null}

              {contact.email ? (
                <a href={`mailto: ${contact.email}`} target="_blank">
                  <Mail className="w-4 h-4" />
                </a>
              ) : null}
            </div>
          </div>
          <Separator className="my-3" />
        </React.Fragment>
      ))}
    </div>
  );
};
